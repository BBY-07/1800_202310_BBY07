...
function generateGameUI(level) {
  ...
  const taskTracker = `<div class="task-tracker">Task ${currentTask + 1}, ${levelTasks[currentTask].name}</div>`;
...
updateUI = () => {
  ...
  taskTracker.innerHTML = `Task ${currentTask + 1}, ${levelTasks[currentTask].name}`
};
